...
function timeSpanToString(startDate, endDate) {
  ...
  const mil = millisec > 99 ? millisec : `0${millisec}`;

  return `${hour > 9 ? hour
    : `0${hour}`}:${min > 9 ? min : `0${min}`}:${sec > 9 ? sec
    : `0${sec}`}.${mil > 9 ? mil : `0${mil}`}`;
}
...
function angleBetweenClockHands(date) {
  ...
  let value = Math.abs(0.5 * (60 * hour - 11 * min));

  if (value > 360) {
    const num = Math.floor(value / 360);
    value -= num * 360;
  }
